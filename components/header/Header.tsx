    ...
    <>
      ...
      <header className="fixed top-0 z-20 w-full transition-colors js-page-header backdrop-blur">
        <div className="flex items-center px-6 py-6 xl:px-24 ">
          <Link className="shrink-0" href="/">
            <a>
              ...
              <div className="hidden dark:block">
                <Image src={WhiteLogo} height={55} width={200} alt="Moondogs | NFT Marketplace" />
              </div>
            </a>
          </Link>
          {/* End  logo */}
          ...
          <div className="fixed inset-0 z-10 items-center invisible ml-auto bg-white opacity-0 js-mobile-menu dark:bg-jacarta-800 lg:visible lg:relative lg:inset-auto lg:flex lg:bg-transparent lg:opacity-100 dark:lg:bg-transparent">
            <nav className="w-full navbar">
              <ul className="flex flex-col lg:flex-row">
                ...
                <li className="relative js-nav-dropdown group">
                  <Link href={home.path}>
                    <a className="dropdown-toggle text-jacarta-700 font-display hover:cursor-pointer hover:text-accent focus:text-accent dark:hover:text-accent dark:focus:text-accent flex items-center justify-between py-3.5 text-base dark:text-white lg:px-5 w-full">
                      <span
                        className={
                          isPageActive(home.path, route.asPath) ? "text-accent  dark:text-accent" : ""
                        }
                      >
                        Home
                      </span>
                ...
                <li className="relative js-nav-dropdown nav-item dropdown group">
                  <Link href={explore.path}>
                    <button className="dropdown-toggle text-jacarta-700 font-display hover:text-accent focus:text-accent dark:hover:text-accent dark:focus:text-accent flex items-center justify-between py-3.5 text-base dark:text-white lg:px-5 w-full">
                      <span
                        className={
                          isPageActive(explore.path, route.asPath) ? "text-accent dark:text-accent" : ""
                        }
                      >
                        Explore
                      </span>
            ...
            <div className="items-center hidden ml-8 lg:flex xl:ml-12">
              <WalletButton />
              {/* End metamask Wallet */}
      ...
      >
        <div className="fixed left-0 z-10 flex items-center justify-between w-full p-6 bg-white t-0 dark:bg-jacarta-800 lg:hidden">
          ...
          <div className="hidden dark:block">
            <Image src={WhiteLogo} height={28} width={130} alt="Moondogs | NFT Marketplace" />
          </div>
          ...
          <ul className="flex flex-col lg:flex-row">
            <li className="relative js-nav-dropdown group">
              <Link href={home.path}>
                ...
                >
                  <span
                    className={isPageActive(home.path, route.asPath) ? "text-accent dark:text-accent" : ""}
                  >
                    {home.name}
                  </span>
                </a>
            ...
            <li className="relative js-nav-dropdown nav-item dropdown group">
              ...
              >
                <span
                  className={isPageActive(explore.path, route.asPath) ? "text-accent dark:text-accent" : ""}
                >
                  {explore.name}
                </span>
                <i className="lg:hidden">
        ...
        <div className="w-full mt-10 lg:hidden">
          <div className="block w-full px-8 py-3 font-semibold text-center text-white transition-all rounded-full js-wallet bg-accent shadow-accent-volume hover:bg-accent-dark">
          ...
          <div className="flex items-center justify-center space-x-5">
            <a className="group">
              <svg
                ...
                data-prefix="fab"
                data-icon="facebook"
                className="w-5 h-5 group-hover:fill-accent fill-jacarta-300 dark:group-hover:fill-white"
                role="img"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 512 512"
              >
